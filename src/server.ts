...
const JOBS: Job[] = []; // In-memory (në prod rekomandohet DB)
...
function parseAccountMap(): Record<string, AccountConfig> {
  ...
  if (!raw) return {};
  try { return JSON.parse(raw); } catch { console.error("Failed to parse IG_ACCOUNT_MAP/JSON"); return {}; }
}
let IG_ACCOUNTS: Record<string, AccountConfig> = parseAccountMap();

/** ========== IG GRAPH HELPERS ========== */
async function createImageContainer(opts: { igUserId: string; imageUrl: string; caption?: string; token: string; }) {
...
/** Reload accounts (rifreskon IG_ACCOUNT_MAP nga env) */
...
app.post("/api/token/debug", requireAdmin, async (req, res) => {
  try { return ok(res, await debugToken((req.body as any).token)); }
  catch (e: any) { return bad(res, 400, e?.response?.data?.error?.message || e?.message || "debug error"); }
});
app.post("/api/token/extend", requireAdmin, async (req, res) => {
  try { return ok(res, await extendUserToken((req.body as any).userToken)); }
  catch (e: any) { return bad(res, 400, e?.response?.data?.error?.message || e?.message || "extend error"); }
});
app.post("/api/token/pages", requireAdmin, async (req, res) => {
  try { return ok(res, { pages: await listUserPages((req.body as any).userToken) }); }
  catch (e: any) { return bad(res, 400, e?.response?.data?.error?.message || e?.message || "pages error"); }
});
app.post("/api/token/page-access", requireAdmin, async (req, res) => {
  try {
    ...
    return ok(res, { pageToken: await getPageAccessToken(pageId, actorToken) });
  } catch (e: any) {
...
app.post("/api/token/ig-from-page", requireAdmin, async (req, res) => {
  try {
    ...
    return ok(res, { igUserId: await getIGFromPage(pageId, pageToken) });
  } catch (e: any) {
